...
def test_output(true_out, pred_grid):
    ...
    true_arr = np.asarray(true_out)
    pred_arr = np.asarray(pred_grid)

    max_rows = min(true_arr.shape[0], pred_arr.shape[0])

    correct = 0
    total   = 0

    for i in range(max_rows):
        row_len = min(true_arr.shape[1] if true_arr.ndim > 1 else len(true_arr[i]),
                      pred_arr.shape[1] if pred_arr.ndim > 1 else len(pred_arr[i]))

        for j in range(row_len):
            gt_val = true_arr[i][j]
            if gt_val != 0:            # count only informative pixels
                total += 1
                if gt_val == pred_arr[i][j]:
                    correct += 1
...
def simulate_gp_run(task_filename, result_dir, run_id):
    ...
    try:
        ...
        gp = Genetic_Prog(POPPULATION_SIZE, GENERATIOON_SIZE, max_depth=5,input_task=input_task)
        best_scores = []
        for g in range(GENERATIOON_SIZE):
                logger.info(f"Generation {g + 1}/{GENERATIOON_SIZE}")
                len_Grid = len(grid_dict["train"])
                training_sample = np.random.randint(0, len_Grid)
                in_task = grid_dict["train"][training_sample]["input"]
                out_task = grid_dict["train"][training_sample]["output"]
                logger.info(f"Training on train example {training_sample}")
                scores = gp.train(in_task, out_task)
                logger.info(f"trying best Individual")
                gp.evaluate_population(in_task,out_task)
                Individual = max(gp.Population, key=lambda ind: ind.score)
                try:
                    logger.info(f"Best individual score: {Individual.score}")
                    ...
                    logger.error(f"Error executing individual: {e}")
                    O = None
                best_scores.append(max(scores))
                

        # Best individual
        best_ind = max(gp.Population, key=lambda ind: ind.score)
...
def run_simulations_on_task(task_file, result_dir_path, num_simulations=10):
...
if __name__ == "__main__":
    ...
    result_dir = "../results"
    runs = 30
    run_simulations_on_task(task_file, result_dir, num_simulations=runs)
